...
class Wp_Rtcamp_Assignment_2a_Ajax_Test extends WP_Ajax_UnitTestCase {
	...
	public function test_if_wprtc_get_single_slide_html_ajax_callback_gives_correct_html() {
		...
		$single_side_html .= '<img  src="' . esc_url( wp_get_attachment_url( $_POST['wprtc_attachment_id'] ) ) . '"  />';
		$single_side_html .= '</div>';
		$single_side_html .= '<div class="wprtc_slide_actions_wrapper">';
		$single_side_html .= '<div class="wprtc_slide_actions">';
		$single_side_html .= '<a href="#" class="wprtc_edit_slide_button" data-slide-order="' . esc_attr( $_POST['wprtc_slide_order'] ) . '">' . esc_html__( 'Edit Slide', 'wprtc_assignment_2a' ) . '</a>';
		$single_side_html .= '<a href="#" class="wprtc_delete_slide_button" data-slide-order="' . esc_attr( $_POST['wprtc_slide_order'] ) . '">' . esc_html__( 'Delete Slide', 'wprtc_assignment_2a' ) . '</a>';
		$single_side_html .= '</div>';
		$single_side_html .= '</div>';
		$single_side_html .= '<input type="hidden" name="_wprtc_slide_order[' . esc_attr( $_POST['wprtc_slide_order'] ) . ']" value="' . esc_attr( $_POST['wprtc_attachment_id'] ) . '" / >';
		$single_side_html .= '</div>';
